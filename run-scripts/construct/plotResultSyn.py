...
from calcResultSyn import constForCalcResult, calcResult
...
class constForPlotResult(constForCalcResult):
	...
	"runtime", "memusage"
	xLabels = ["Uniform (mean)", "Normal (\mu)", "Normal (\sigma)", "Exponential (1/\lambda)"]
...
def genFigure(srcFilePath, desFileName):
	xLists = []
	xLists.append(map(str, [0.1, 0.3, 0.5, 0.7, 0.9]))
	xLists.append(map(str, [0.1, 0.3, 0.5, 0.7, 0.9]))
	xLists.append(map(str, [0.10, 0.15, 0.20, 0.25, 0.3]))
	xLists.append(map(str, [0.1, 0.3, 0.5, 0.7, 0.9]))
