...
from calcResultReal import constForCalcResult, calcResult
...
class constForPlotResult(constForCalcResult):
	...
	"runtime", "memusage"
	xLabels = ["NYC (k)", "Tokyo (k)", "Haikou (k)", "Chengdu (k)"]
...
def genFigure(srcFilePath, desFileName):
	xLists = []
	xLists.append(map(str, CFPR.alphas))
	xLists.append(map(str, CFPR.alphas))
	xLists.append(map(str, CFPR.alphas))
	xLists.append(map(str, CFPR.alphas))
